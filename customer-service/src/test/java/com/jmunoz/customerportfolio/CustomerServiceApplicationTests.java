package com.jmunoz.customerportfolio;
...
class CustomerServiceApplicationTests {
    ...
    public void buyAndSell() {
        ...
        // check de los holdings.
        // No deben haber dos entradas, sino una entrada que se crea y luego se actualiza.
        getCustomer(2, HttpStatus.OK)
                .jsonPath("$.holdings").isNotEmpty()
                .jsonPath("$.holdings.length()").isEqualTo(1)   // intencional aunque parezca redundante
    ...
    // Escenarios de error.
    // Obtenemos objetos ProblemDetail
    @Test
    ...
    // Con este méto-do genérico GET podemos centrarnos en la parte de aserciones.
    private WebTestClient.BodyContentSpec getCustomer(Integer customerId, HttpStatus expectedStatus) {
        return this.client.get()
                ...
                .expectBody()
                // Imprimimos la respuesta que obtenemos.
                .consumeWith(e -> log.info("{}", new String(Objects.requireNonNull(e.getResponseBody()))));
    }

    // Con este méto-do genérico POST podemos centrarnos en la parte de aserciones.
